...
def open_company_file(start):
    with open("file_cs-uri-stem_company_W3SVC.txt", 'r') as r:
...
def restart():
    with open("company_title.txt", "r") as r:
...
def write_company_title():
    with open('company_title.txt', 'a') as w:
        w.writelines("12321\n")
        w.writelines("asd")
...
if __name__ == '__main__':
    with open('company_title.txt', 'a') as w:
        ...
        print(line_numbers)
        url_list = open_company_file(line_numbers)
        for i in url_list:
